...
export default function DecorativeEllipsis({
  ...
  return (
    <div className={twMerge("flex gap-1.5 lg:gap-3", className)}>
      <motion.span
        key={`${uniqueKey}-1`}
        initial="hidden"
        whileInView="visible"
      ...
      <motion.span
        ...
        className="size-3 lg:size-4 rounded-full bg-[linear-gradient(167.47deg,_var(--color-brown)_9.09%,_var(--color-black)_105.68%)]"
      />
      <motion.span
        key={`${uniqueKey}-3`}
        initial="hidden"
        ...
        className="size-3 lg:size-4 rounded-full bg-black"
      />
    </div>
  );
}
